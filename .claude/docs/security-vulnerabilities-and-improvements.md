...
No frontend npm vulnerabilities were reported by `npm audit`. Python dependency audit tooling and local secret scanning were not available in this workstation environment, so those results should be treated as incomplete until CI or a machine with `pip-audit` and `gitleaks` installed reruns them.

Remediation has started. Several code-level controls are now implemented; project-level items such as GCP Workload Identity Federation, image digest pinning, and formal data-retention policy still require deployment or product decisions.
...
| High | Segmentation service auth fallback | Mitigated in code; requires production env | `REQUIRE_SEGMENTATION_IAM` now lets production fail closed if the backend cannot fetch a Cloud Run identity token. Default remains false for local development. | Production can avoid silent unauthenticated fallback once the flag is enabled. | Set `REQUIRE_SEGMENTATION_IAM=true` in production and validate `SEGMENTATION_MS_URL` is HTTPS. |
...
| Low | Frontend security headers incomplete | Mitigated for Nginx image | `nginx.conf` now includes HSTS and restrictive Permissions-Policy in addition to CSP, nosniff, referrer policy, and frame protections. | Docker/Nginx frontend deployments have stronger default browser protections. | Confirm equivalent headers in Firebase Hosting config if it bypasses Nginx. |
...
|---|---|---|
| Frontend | `npm audit --json` in `car-frontend` | Passed with 0 vulnerabilities across 442 total dependencies. |
...
- Set `REQUIRE_SEGMENTATION_IAM=true` in production and consider adding a broader `ENVIRONMENT=production` setting for future production-only checks.
- Add Firebase Storage emulator tests for the existing `car-frontend/storage.rules`, then run them in CI.
- Add structured security logs around denied uploads, segmentation failures, OpenAI failures, and quota/rate-limit events.
...
1. Backend: enable `REQUIRE_SEGMENTATION_IAM=true` in production deployment config.
2. Backend: replace in-memory edit rate limiting with durable cross-instance quotas.
3. Segmentation: replace base64 JSON image return with a lower-overhead response format.
4. Storage: add emulator-based tests for Firebase Storage rules.
5. CI/CD: replace `GCP_SA_KEY` with Workload Identity Federation.
6. Dependencies: update local Conda envs and rerun `pip-audit`.
7. Frontend: remove `unsafe-inline` from CSP if feasible.
8. Docker/Cloud Run: pin base image digests and document runtime hardening settings.
9. Privacy: define retention rules for originals, results, masks, and prompts.
10. Observability: add structured security/cost logs for denied uploads, quotas, and provider failures.
